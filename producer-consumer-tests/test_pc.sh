...
WORKLOADS_DIR="${PROJECT_ROOT}/workloads/pc-tests"
...
# ========== 主测试循环 ==========
while [ $PRODUCERS_PER_TOPIC -le $MAX_PRODUCER ]
do
    CONSUMERS_PER_TOPIC=$PRODUCERS_PER_TOPIC
    PER_PRODUCER_RATE=$((PRODUCER_RATE / PRODUCERS_PER_TOPIC))

    PC_DIR="${RESULTS_BASE_DIR}/pc_${PRODUCERS_PER_TOPIC}p_${CONSUMERS_PER_TOPIC}c"
    mkdir -p "$PC_DIR"

    echo "=============================="
    echo " 测试场景: ${PRODUCERS_PER_TOPIC} Producer(s) ↔ ${CONSUMERS_PER_TOPIC} Consumer(s)"
    echo "=============================="
    echo "结果目录: $PC_DIR"
    echo ""
    ...
    do
        NAME="${BASE_NAME} - ${PRODUCERS_PER_TOPIC}p-${CONSUMERS_PER_TOPIC}c - Run ${iteration}"
        echo "------------------------------"
        echo " Run ${iteration}/${ITERATIONS}"
        echo "------------------------------"

        CONFIG_FILE_NAME="pc_${PRODUCERS_PER_TOPIC}p_${CONSUMERS_PER_TOPIC}c_run${iteration}.yaml"
...
producerRate: ${PER_PRODUCER_RATE}
subscriptionsPerTopic: ${SUBSCRIPTIONS_PER_TOPIC}
consumerPerSubscription: ${CONSUMERS_PER_TOPIC}
...
EOF
    ...
    echo "🎯 当前轮次完成: ${PRODUCERS_PER_TOPIC}p-${CONSUMERS_PER_TOPIC}c"
    echo ""
    PRODUCERS_PER_TOPIC=$((PRODUCERS_PER_TOPIC * 2))
done
